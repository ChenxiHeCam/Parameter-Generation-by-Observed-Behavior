...
full5 recovery (\ref{sec:hero_full5}) & BAAIWorm & scramble $s{=}100$ & sim $\theta^{*}$ (SOTA) & $\approx$5k (per-element full5) & SPSA$\times$60--140$+$CMA$+$GAN & food 50--69, 160 cond \\
...
\begin{table}[H]
\centering\small
\caption{Claim-1 parameter recovery (PGOB-sim, ground truth $\theta^{*}$ known). Because the target is the simulator's own $\theta^{*}$, the recovery is verifiable directly in parameter space: $\mathrm{rel}_\theta{=}\lVert\hat\theta-\theta^{*}\rVert/\lVert\theta^{*}\rVert$ from a scale-$0.1$ randomised start. modWorm is the tightest recovery of any simulator ($\mathrm{rel}_\theta{=}0.009$), confirming that behaviour-only generation reconstructs the exact ground-truth physiology. BAAIWorm full5 is reported on its behaviour band (best held-out $0.1199$, within the published SOTA band $0.120$--$0.125$) because its full-connectome $\theta^{*}$ recovery is judged in behaviour space.}
\label{tab:claim1_recovery}
\begin{tabular}{lccl}\toprule
Simulator & Dim & Parameter recovery $\mathrm{rel}_\theta$ & Behaviour recovery \\\midrule
...
\caption{\textbf{Multi-arm hero case study (fly, FlyGym v2 $\to$ Janelia).} \emph{(i)} Aggregate distance to real Janelia ($10$ observables; lower $=$ more real) across a three-segment recovery, random actuator-gain start $\to$ PGOB $\to$ expert default ($13.30\to10.78\to10.64$, $N{=}20$/arm), recovering $94.9\%$ of the random-to-expert gap; the random start fails to walk in $8/20$ rollouts. \emph{(ii)} The forward- and lateral-speed channels, with random start, PGOB and expert side by side: PGOB collapses almost onto expert. The framework-integrity reading (PGOB-sim recovering the simulator's own $\theta^{*}$ to a held-out distance of $3.42$) is in Table~\ref{tab:modes_v8}, on its own consistent scale.}
