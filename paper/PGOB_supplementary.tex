...
\label{si:freshbuild}
To verify that the headline results do not depend on any pre-existing artifact, all four simulators were rebuilt from public source on commodity CPU hardware and the behaviour-recovery runs reproduced live, relying on nothing carried over from earlier environments. \textbf{BAAIWorm}: a headless CPU inference engine was compiled from source from \texttt{github.com/Jessie940611/BAAIWorm} by removing the GPU OptiX/OWL renderer dependency and linking only the FEM and physics modules; the resulting \texttt{interact.so} imports with no CUDA/OptiX and runs the full closed loop (the base connectome weights ship inside the repository). \textbf{FlyGym}: fresh \texttt{pip install flygym} (1.2.1, mujoco 3.2.7) in a clean environment. \textbf{modWorm}: fresh clone of the Cook-ODE model, run through its documented Python integrator backend (the Julia DifferentialEquations path hit a known Julia~1.10.4 precompilation bug and was bypassed without altering the model). \textbf{flybody}: fresh clone (0.1.0, mujoco~3.2.3). In every case the live values reproduced the deposited genuine results.

\textbf{BAAIWorm per-feature within-variance (live).} On the single cleanly comparable non-artifact locomotion feature (forward-state propensity; real OWMD-N2 band $[0.032,0.638]$), all four arms fall within or at the lower edge of the real band (world-frame: default $0.084$, PGOB-sim $0.059$, PGOB-real $0.025$, PGOB-hybrid $0.050$). The verdict is frame-dependent at the margin, so we read this as biological plausibility / parity rather than a decisive ``PGOB enters the real variance band'' claim; PGOB-sim substantially recovers the sim target. The remaining worm channels are excluded as non-comparable: a backward-state labelling-convention artifact and a turn-magnitude unit mismatch between the live per-step extractor and the stored OWMD band. We therefore do not claim that the hybrid arm decisively enters the real individual-variance band.
